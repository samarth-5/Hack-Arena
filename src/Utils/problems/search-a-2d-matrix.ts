...
type SearchMatrixFunction = (matrix: number[][], target: number) => boolean;

export const search2DMatrixHandler = (fn: SearchMatrixFunction): boolean => {
  try {
    const tests = [
      {
        matrix: [
...
export const search2DMatrix: Problem = {
  ...
  handlerFunction: search2DMatrixHandler as (fn: unknown) => boolean,
